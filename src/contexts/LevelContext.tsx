...
export const LevelProvider: React.FC<LevelProviderProps> = ({ children }) => {
  ...
  const nextLevel = (choice: string) => {
    setV(() => {
      if (choice == "left")
        return waterVolume + levelData[level].getleftscore.waterVolume;
      else return waterVolume + levelData[level].getrightscore.waterVolume;
    });
    setQ(() => {
      if (choice == "left")
        return waterQuality + levelData[level].getleftscore.waterQuality;
      else return waterQuality + levelData[level].getrightscore.waterQuality;
    });
    setT(() => {
      if (choice == "left")
        return temperature + levelData[level].getleftscore.temperature;
      else return temperature + levelData[level].getrightscore.temperature;
    });
    setE(() => {
      if (choice == "left")
        return (
          engineeringBudget + levelData[level].getleftscore.engineeringBudget
        );
      else
        return (
          engineeringBudget + levelData[level].getrightscore.engineeringBudget
        );
    });
    setLevel((prevLevel) => {
      console.log(choice);
